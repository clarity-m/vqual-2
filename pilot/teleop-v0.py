...
ACRO_PITCH = 2.5
ACRO_YAW = 2.0
THRUST_HOVER = 0.55       # UNVERIFIED hover point; trim in flight with up / down
...
KEYS_MODIFIER = {"boost": "ctrl", "precision": "alt"}
...
class Pilot:
    ...
    # -- streamed setpoints -----------------------------------------------------
    def send(self, axes, scale, dt, armed, heading=0.0):
        ...
        # Throttle is a held value, not a stick deflection - it integrates while
        # a key is down and stays put when released.
        self.thrust = min(1.0, max(0.0, self.thrust + thr_ax * THRUST_SLEW * dt))
...
def draw_hud(img, pilot, tel, vision, marker_count, heading_now):
    ...
    line1 = "RATES r%+.2f p%+.2f y%+.2f  THR %.2f  hdg %+4.0f%s" % (
        a, b, c, d, math.degrees(heading_now),
...
def console_line(pilot, tel, vision, heading_now):
    a, b, c, d = pilot.last_cmd
    sys.stdout.write(
        "\r%-8s r%+.2f p%+.2f y%+.2f thr%.2f hdg%+4.0f gate %-3s t%6.1f "
        "contact %-3d cam %4.1f fps imu %-6d " % (
            "ARMED" if tel["armed"] else "disarmed",
            a, b, c, d, math.degrees(heading_now),
            tel["active_gate"] if tel["active_gate"] >= 0 else "-",
